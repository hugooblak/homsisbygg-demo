...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'homsisbygg-leads' ),
					'singular_name' => __( 'Lead', 'homsisbygg-leads' ),
					'edit_item'     => __( 'Förfrågan', 'homsisbygg-leads' ),
					'search_items'  => __( 'Search leads', 'homsisbygg-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'homsisbygg-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'homsisbygg-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'homsisbygg-leads' ),
			'npl_phone'    => __( 'Phone', 'homsisbygg-leads' ),
			'npl_service'  => __( 'Needs', 'homsisbygg-leads' ),
			'npl_timeline' => __( 'When', 'homsisbygg-leads' ),
			'npl_zip'      => __( 'ZIP', 'homsisbygg-leads' ),
			'npl_source'   => __( 'Source', 'homsisbygg-leads' ),
			'npl_status'   => __( 'Status', 'homsisbygg-leads' ),
			'date'         => __( 'Received', 'homsisbygg-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'homsisbygg-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'homsisbygg-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'homsisbygg-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'homsisbygg-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'homsisbygg-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'homsisbygg-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'homsisbygg-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'homsisbygg-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'homsisbygg-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'homsisbygg-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'homsisbygg-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'homsisbygg-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
