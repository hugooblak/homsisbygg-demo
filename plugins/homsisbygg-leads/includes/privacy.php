...
add_filter(
	...
	function ( $exporters ) {
		$exporters['homsisbygg-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'homsisbygg-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'homsisbygg-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'homsisbygg-leads',
						'group_label' => __( 'Quote requests', 'homsisbygg-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['homsisbygg-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'homsisbygg-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
